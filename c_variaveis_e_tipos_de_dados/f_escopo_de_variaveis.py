"""
Escopo de variáveis

Dois casos de escopo:

1 - Variáveis Globais:
     - Variáveis Globais são reconhecidas, ou seja, seu escopo compreende, todo o oprograma.

2 - Variáveis Locais:
     - Variáveis Locais são reconhecidas apenas no bloco onde foram declaradas, ou seja,
     seu escopo está limitado ao bloco onde foi declarada.

Para declarar variáveis em Python fazemos:

nome_da_variavel = valor_da_variavel

Python é uma lingyuagem de tipagem dinâmica. Isso significa que
ao declarar-mos uma variavel, nós não colocamos o tipo de dado dela.
Este tipo é inferido ao atribuirmos o valor a mesma.

Exemplo em C:
int numero = 42;

Exemplo em Java:
int numero = 42;
"""

numero = 42  # Exemplo de variável global
print(numero)
print(type(numero))

numero = 'Geek'
print(numero)
print(type(numero))

nao_existo = 'Oi'
print(nao_existo)

numero = 42
# novo = 0

if numero > 10:
    novo = numero + 10 # A variavel 'novo' esta declarada localmente dentro do bloco do if. Portanto, é local
    print(novo)

print(novo)
